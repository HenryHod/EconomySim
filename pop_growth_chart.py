...
data.append(go.Scatter3d(x=df["savings_rate"][min_indexes_est], y=df["cons_rate"][min_indexes_est], z=df["char_rate"][min_indexes_est], mode='markers',marker=dict(color="green", size=4), opacity=0.5,
hovertemplate = "Savings Rate: %{x}<br>" +
                        "Consumption Rate: %{y}<br>" +
                        "Charity Rate %{z}<br>" +
                        "Population Growth: %{marker.color:,}" +
