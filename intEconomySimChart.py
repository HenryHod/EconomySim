...
app.layout = html.Div([
    ...
    dcc.Slider(
        id='endowment-slider',
        min = childcost, max = childcost * 20, step = childcost * 2,
        value = childcost * 10),
    html.H4("Charity"),
    dcc.Slider(
        id='charity-slider',
...
def set_scale(scale_input, endow_input, char_input, skills_input):
    ...
    SELECT altruism, impatience, 
        (CAST((new_children * 20) AS DOUBLE) / (good1 + good2)) AS child_per_dollar, 
        (future_good1 + future_good2) AS total_goods, 
        (CAST((self_good1 + self_good2) AS DOUBLE) / (good1 + good2)) AS mpc FROM simulations
    WHERE good1_pref + good2_pref < {scale_input + 0.1} AND good1_pref + good2_pref > {scale_input - 0.1} 
    AND generation == 0
    AND good1 + good2 < {endow_input + childcost} AND good1 + good2 > {endow_input - childcost}
    AND charity < {char_input + 0.1} AND charity > {char_input - 0.1}
    AND skills == {skills_input}
    """).fetchall(), columns = ["altruism", "impatience", "child_per_dollar", "total_goods", "mpc"])
    lock.release()
    print(scale_df.head())
    x = scale_df["altruism"]
