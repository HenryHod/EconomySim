...
print(df)
per_vars = df.groupby("sim_id").var().reset_index().drop("period", axis=1)
per_vars["sample_size"] = per_vars["sim_id"].apply(lambda x: samples[x])
vars = per_vars.groupby("sample_size").mean().reset_index()
print(vars)
